...
const OBJ_Q = ['policy_get_policies', 'policy_get_rules', 'rule_get_implementations', 'platform_get_policies', 'platform_get_rules', 'platform_get_job_types', 'job_type_get_platform', 'rule_impl_get_job_type', 'model_get_fields', 'model_get_jobs']
const FUNC = ['get_anchored_object', 'get_object_type', 'get_links', 'list_functions', 'list_actions', 'classify_job', 'policies_for', 'rules_for', 'get_scan_plan', 'check_consistency', 'get_lineage_upstream', 'get_lineage_downstream', 'get_impact', 'explain_finding']
check('14 通用 Function + 10 对象类型查询 + 1 Action 注册', [...FUNC, ...OBJ_Q, 'propose'].every((n) => tools.has(n)))
check('Function risk=read', FUNC.every((n) => tools.get(n)?.risk === 'read'))
check('Action propose risk=knowledge-write', tools.get('propose')?.risk === 'knowledge-write')
// 本体四大能力（Palantir 范式）
const ty = call('get_object_type', { type: 'JobType' })
...
const tyPol = call('get_object_type', { type: 'Policy' })
...
const lkType = call('get_links', { target: 'JobType' })
...
const lkInst = call('get_links', { target: 'jt/schema-change' })
...
check('functions() 目录含 get_object_type/get_links/get_scan_plan', (() => { const f = call('list_functions', {}); return f.ok === true && f.functions.some((x) => x.name === 'get_object_type') && f.functions.some((x) => x.name === 'get_links') && f.functions.some((x) => x.name === 'get_scan_plan') })())
check('actions() 目录含 propose(gated)', (() => { const a = call('list_actions', {}); return a.ok === true && a.actions.some((x) => x.name === 'propose') })())
// 对象类型作用域查询（具名工具，名即 ObjectType.method）：Policy.getPolicies(platform) / Policy.getRules(policy) / Rule.getImplementations(rule)
const qP = call('policy_get_policies', { platform: 'inst/dbscript' })
...
const qR = call('policy_get_rules', { policy: 'pol/design-consistency' })
...
const qI = call('rule_get_implementations', { rule: 'rule/rc@dbscript-field-type' })
...
const qJP = call('job_type_get_platform', { jobType: 'jt/schema-change' })
check('JobType.getPlatform(jt/schema-change) → inst/dbscript', qJP.ok === true && qJP.platform && qJP.platform.id === 'inst/dbscript', JSON.stringify(qJP.platform))
// 本体推理入口：锚定驱动——先找「当前锚定的对象」（读 workspace 锚点，非手传 path）
const ao = call('get_anchored_object', {})
...
const ao2 = call('get_anchored_object', {})
...
check('scan_plan 给出怎么扫（engines + 执行器）', plan.engines && plan.engines.includes('se-sql 1.8') && /check_consistency/.test(plan.howToScan || ''), plan.howToScan)
...
check('reasonChain 推理方向单向边：instanceOf/hasJobType/covers/containsRule/implementedBy（无回边）', ['instanceOf', 'hasJobType', 'covers', 'containsRule', 'implementedBy'].every((t) => (chain.edges || []).some((e) => (e.label || '').startsWith(t))) && !(chain.edges || []).some((e) => (e.label || '').startsWith('appliesTo') || (e.label || '').startsWith('bindsJobType')), JSON.stringify((chain.edges || []).map((e) => e.label)))

// 3. classify_job：特征推理（从 repo 读 ETL 作业，非查预建）
const c1 = call('classify_job', { path: 'etl/dwd/dwd_tax_declaration.etl' })
check('classify ETL 作业 → jt/process-bds（特征推理）', c1.ok === true && c1.jobType === 'jt/process-bds', c1.jobTypeName + '/' + c1.instanceName)
const c2 = call('classify_job', { path: 'dbscript/alter_dwd_tax_payment_v4.sql' })
...
const m1 = call('check_consistency', { path: 'etl/dwd/dwd_tax_declaration.etl' })
...
const m2 = call('check_consistency', { path: 'dbscript/alter_dwd_tax_payment_v4.sql' })
...
const up = call('get_lineage_upstream', { path: 'etl/dwd/dwd_tax_declaration.etl' })
check('lineage_upstream 可调用', up.path === 'etl/dwd/dwd_tax_declaration.etl')
const imp = call('get_impact', { model: 'dwd_tax_declaration.model' })
...
const pol = call('policies_for', { jobType: 'jt/schema-change' })
check('policies_for 跨平台策略', (pol.policies || []).length >= 3, (pol.policies || []).map((p) => p.name).join(','))
const rl = call('rules_for', { jobType: 'jt/schema-change' })
check('rules_for 规则+impl 绑定', rl.ruleCount >= 3 && rl.rules.some((r) => r.impl && r.impl.ruleset === 'se-sql 1.8'))
const e1 = call('explain_finding', { finding: 'F-101' })
check('explain_finding 归因链', e1.chain.includes('字段类型一致性'))
check('propose 三 kind', ['proposal', 'assertion', 'diverge-ruling'].every((k) => call('propose', { kind: k, payload: {}, approvalNote: '确认' }).kind === k))
