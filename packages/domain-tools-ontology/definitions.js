...
export function buildDefinitions(p, { repo, modeling, anchor } = {}) {
  ...
  return [
    ...
    // 每个 ObjectType 的方法各成一个具名工具，工具名即「ObjectType.method」，
    // 会话流里打印出来就是 Policy.getPolicies(inst/dbscript) —— 不是笼统的 ontology_object_query。
    { name: 'Policy.getPolicies', risk: 'read', category: FUNC,
      description: '【Ontology ObjectType 查询】Policy.getPolicies(platform)：沿 covers(逆向) 取覆盖某平台实例的策略（治理目标）。如 Policy.getPolicies(inst/dbscript) → 数据库脚本平台的 3 组策略。先 ontology_anchored_object + ontology_links 拿到 platform，再调本方法',
    ...
    { name: 'Policy.getRules', risk: 'read', category: FUNC,
    ...
    { name: 'Rule.getImplementations', risk: 'read', category: FUNC,
    ...
    { name: 'PlatformInstance.getPolicies', risk: 'read', category: FUNC,
    ...
    { name: 'PlatformInstance.getRules', risk: 'read', category: FUNC,
    ...
    { name: 'PlatformInstance.getJobTypes', risk: 'read', category: FUNC,
    ...
    { name: 'JobType.getPlatform', risk: 'read', category: FUNC,
    ...
    { name: 'RuleImpl.getJobType', risk: 'read', category: FUNC,
    ...
    { name: 'Model.getFields', risk: 'read', category: FUNC,
    ...
    { name: 'Model.getImplementingJobs', risk: 'read', category: FUNC,
    ...
    { name: 'ontology_policies_for', risk: 'read', category: FUNC,
      description: '【Ontology Function · 本体语义推理】取作业类型适用的治理目标（JobType → PlatformInstance →covers 逆向→ Policy，跨平台）。是 Policy.getPolicies(platform) 的作业类型糖：先 ontology_anchored_object 拿 jobTypes，再据此取策略。本体的只读推理，非普通执行工具',
      parameters: { type: 'object', properties: { jobType: S('string', { description: '作业类型 id，如 jt/schema-change' }) }, required: ['jobType'] },
