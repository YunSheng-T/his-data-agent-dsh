...
export function buildDefinitions(p, { repo, modeling, anchor } = {}) {
  ...
  return [
    { name: 'ontology_anchored_object', risk: 'read', category: FUNC,
      description: '【Ontology Function · 本体推理入口｜必须先调】读当前工作区锚定的对象（workspace-anchor 的 current：建模空间模型文件，或代码仓分支+作业目录），回答「当前锚定的对象是什么」——五原语 ObjectType（Model / Directory / Job…）+ 在本体图中的直接关系（hasField / implements / inDirectory / instanceOf）+ 可达的作业类型 jobTypes。这是本体推理的起点：拿到锚定对象后，用 ontology_type（看 ObjectType 定义）与 ontology_links（关系遍历）沿本体边找 Platform/Policy/Rule，再用 ontology_object_query（如 Policy.getPolicies(platform) / Rule.getImplementations(rule)）查策略规则，最后 ontology_scan_plan 决定扫什么、怎么扫。不要跳过锚定对象直接调用结果型 function。返回的 why 就是识别依据；**调用后必须在回复中引用识别依据**（如「锚定=代码仓 main/dbscript → 作业 1 个 → instanceOf 表结构变更」），让用户看得懂「怎么锚定到、怎么识别到」，不要只报结论',
      parameters: { type: 'object', properties: {}, required: [] },
      output: jsonOut, execute: () => p.anchoredObject(ctx) },
    { name: 'ontology_type', risk: 'read', category: FUNC,
    ...
    { name: 'ontology_links', risk: 'read', category: FUNC,
    ...
    { name: 'ontology_object_query', risk: 'read', category: FUNC,
      description: '【Ontology Function · 对象类型作用域查询】Policy.getPolicies(platform) / Policy.getRules(policy) / Rule.getImplementations(rule) / PlatformInstance.getPolicies·getRules·getJobTypes / JobType.getPlatform·getPolicies·getRules / RuleImpl.getJobType / Model.getFields·getImplementingJobs。这是本体关系查询能力：先 ontology_type 看某类型有哪些 query 可用，再本函数执行。如 Policy.getPolicies(inst/dbscript) 沿 covers 逆向取数据库脚本平台的策略，Policy.getRules(pol/design-consistency) 沿 containsRule 取该策略下的规则。先 ontology_anchored_object + ontology_links 定位到 Platform/Policy/Rule，再据此查',
      parameters: { type: 'object', properties: { objectType: S('string', { description: 'ObjectType，如 Policy / Rule / PlatformInstance / JobType / RuleImpl / Model' }), method: S('string', { description: '查询方法，如 getPolicies / getRules / getImplementations / getPlatform / getJobTypes / getFields / getImplementingJobs' }), arg: S('string', { description: '参数 id，如 inst/dbscript / pol/design-consistency / rule/rc@dbscript-field-type' }) }, required: ['objectType', 'method', 'arg'] },
      output: jsonOut, execute: (args) => p.objectQuery(args.objectType, args.method, args.arg, ctx) },
    { name: 'ontology_functions', risk: 'read', category: FUNC,
    ...
    { name: 'ontology_actions', risk: 'read', category: FUNC,
    ...
    { name: 'ontology_policies_for', risk: 'read', category: FUNC,
      description: '【Ontology Function · 本体语义推理】取作业类型适用的治理目标（JobType → PlatformInstance →covers 逆向→ Policy，跨平台）。是 ontology_object_query(Policy.getPolicies(platform)) 的作业类型糖：先 ontology_anchored_object 拿 jobTypes，再据此取策略。本体的只读推理，非普通执行工具',
    ...
    { name: 'ontology_rules_for', risk: 'read', category: FUNC,
      description: '【Ontology Function · 本体语义推理】取平台实例适用的规则及实现（PlatformInstance →appliesTo 逆向→ Rule →implementedBy→ RuleImpl，按作业类型匹配实现）。是 ontology_object_query(PlatformInstance.getRules / Rule.getImplementations) 的作业类型糖。规则只声明，执行在扫描引擎。本体的只读推理，非普通执行工具',
      parameters: { type: 'object', properties: { jobType: S('string', { description: '作业类型 id，如 jt/schema-change' }) }, required: ['jobType'] },
