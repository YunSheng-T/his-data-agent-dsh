...
export const FUNCTIONS = [
  ...
  { name: 'ontology_policy_get_policies', risk: 'read', returns: '平台策略', purpose: 'Policy.getPolicies(platform) · covers(逆向) 取覆盖某平台的策略' },
  { name: 'ontology_policy_get_rules', risk: 'read', returns: '策略下规则', purpose: 'Policy.getRules(policy) · containsRule 取策略下的规则' },
  { name: 'ontology_rule_get_implementations', risk: 'read', returns: '规则实现', purpose: 'Rule.getImplementations(rule) · implementedBy 取规则实现（engine 指向执行器）' },
  { name: 'ontology_platform_get_policies', risk: 'read', returns: '平台策略', purpose: 'PlatformInstance.getPolicies(platform) · covers(逆向) 从平台侧取策略' },
  { name: 'ontology_platform_get_rules', risk: 'read', returns: '平台规则', purpose: 'PlatformInstance.getRules(platform) · appliesTo(逆向) 取平台规则' },
  { name: 'ontology_platform_get_job_types', risk: 'read', returns: '平台作业类型', purpose: 'PlatformInstance.getJobTypes(platform) · hasJobType 取平台作业类型' },
  { name: 'ontology_job_type_get_platform', risk: 'read', returns: '归属平台', purpose: 'JobType.getPlatform(jobType) · hasJobType(逆向) 取作业类型归属平台' },
  { name: 'ontology_rule_impl_get_job_type', risk: 'read', returns: '绑定作业类型', purpose: 'RuleImpl.getJobType(impl) · bindsJobType 取实现绑定的作业类型' },
  { name: 'ontology_model_get_fields', risk: 'read', returns: '模型字段', purpose: 'Model.getFields(model) · hasField 取模型字段' },
  { name: 'ontology_model_get_implementing_jobs', risk: 'read', returns: '实现作业', purpose: 'Model.getImplementingJobs(model) · implements(逆向) 取实现模型的作业' },
