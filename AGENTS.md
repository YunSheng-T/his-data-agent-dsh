...
- 场景链路固定：`ops_screen` 筛清单 → 回显口径（含自动排除项）→ `ops_topo` 血缘排序 → `ops_gen` 成对生成 → `ops_check` 自检 → `repo_commit` 提交 → `ops_deploy` 部署 → `ops_callback` 解读回调。**顺序由血缘推导，禁止手写作业顺序**。
- 三道门话术：①`ops_gen` 前先把目标清单+排除项讲清楚（清单确认）；②`repo_commit` 前讲执行序列方向与豁免告警（序列确认）；③`ops_deploy` 前必须向用户索取变更单号，执行模式默认 MANUAL（部署确认）。每道门等审批结果，被拒即停手汇报。
- 依赖完整性告警（如核心报表依赖被暂停的作业）必须如实告知并获知情确认，不得静默带过。
